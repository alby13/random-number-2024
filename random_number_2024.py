...
class RandomNumberGenerator:
    ...
    def generate_os_urandom_hardware_random(self, min_value: int, max_value: int) -> int:
...
class RandomNumberGeneratorGUI:
    ...
    def create_widgets(self):
        ...
        self.on_random_frame = self.create_generator_frame(main_frame, "Operating System Hardware Generator", 2)
    ...
    def create_generator_frame(self, parent, title, column):
        ...
        description = ""
        if "Entropy" in title:
            description = "This method uses the operating system's entropy pool to generate a random number. The entropy pool is a collection of random data gathered from various sources, such as keyboard and mouse input, network packets, and disk I/O. This method is considered to be highly random and secure."
        elif "Intel" in title:
            description = "This method uses Intel's Hardware Random Number Generator (HRNG) to generate a random number. The HRNG is a hardware-based random number generator that uses thermal noise to generate random numbers. This method is considered to be highly random and secure, but it is only available on Intel processors."
        elif "Operating" in title:
            description = "This method uses the operating system's urandom function to generate a random number. The urandom function uses a combination of hardware and software-based random number generators to generate random numbers. This method is considered to be highly random and secure."
        elif "AMD" in title:
            description = "This method uses AMD's Hardware Random Number Generator (HRNG) to generate a random number. The HRNG is a hardware-based random number generator that uses thermal noise to generate random numbers. This method is considered to be highly random and secure, but it is only available on AMD processors."

        ttk.Label(frame, text=description, wraplength=200).grid(row=0, column=0, columnspan=2, padx=5, pady=5)

        ttk.Label(frame, text="Min:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        ...
        ttk.Label(frame, text="Max:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        ...
        if "Intel" in title:
            ...
            generate_func = self.generate_os_urandom_hardware_random
        elif "AMD" in title:
    ...
    def generate_os_urandom_hardware_random(self, min_value, max_value):
        try:
            # Generate random number using os.urandom
            random_bytes = os.urandom(8)
            random_long = struct.unpack('Q', random_bytes)[0]
            return min_value + (random_long % (max_value - min_value + 1))
